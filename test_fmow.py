...
import model
from dataset import FMoWSentinel2Path, FSAllPatch
...
def test_loader(args, g_ema, device):
    ...
    testset = FMoWSentinel2Path(args.test_path, transform=transform, enc_transform=enc_transform,
...
#         d_module = discriminator
    ...
    with torch.no_grad():
        ...
        for idx in pbar:
            ...
            utils.save_image(
                sample,
                os.path.join(path, 'cls', args.output_dir, img_path[0]+".png"),
                nrow=1,
                normalize=True,
                range=(-1, 1),
            )
...
def test_patch_loader(args, g_ema, device):
    ...
    testset = FSAllPatch(args.test_path, transform=transform, enc_transform=enc_transform,
...
#                 break
            ...
            filename = os.path.join(path, 'cls', args.output_dir, img_path[0].replace("tif", "png").replace("jpg", "png"))

            if os.path.exists(filename) and i<21500:
                continue
            else:
